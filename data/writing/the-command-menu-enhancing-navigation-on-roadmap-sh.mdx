...
- **Instant Search:** The Command Menu enables users to search for specific pages, roadmaps, guides, or other content directly from the menu. As users type their search queries, the Command Menu dynamically updates the search results, making it easier to find the desired information.

- **Quick Access to Pages:** Users can quickly access important pages, such as the Home, Account, Roadmaps, Best Practices, Guides, and Videos pages, directly from the Command Menu. Each page is associated with a title, group, and an optional icon, providing a visually appealing and organized menu structure.
...
- **Protection of Sensitive Pages:** The Command Menu takes into account user authentication and access control. Protected pages, such as the Account page, are only displayed if the user is logged in. This ensures that sensitive information remains secure and inaccessible to unauthorized users.
  ...
  getAllPages().then((unfilteredPages = defaultPages) => {
    const filteredPages = unfilteredPages
      .filter((currPage: PageType) => {
        return (
          currPage.title.toLowerCase().indexOf(normalizedSearchText) !==
          -1
        );
      })
      .slice(0, 10);
...
You can check out the full source code in the [GitHub](https://github.com/kamranahmedse/developer-roadmap/blob/master/src/components/CommandMenu/CommandMenu.tsx). Also we'll improve this in the future, so stay tuned!
